...
@Composable
private fun Title(
    title: String = "Lo lamentamos por el momento nuestros servidores no se encuentran disponibles"
) {
    Text(
        text = title,
...
private fun Close() {
    Image(
        ...
        contentDescription = "",
        modifier = Modifier.size(100.dp).testTag(Constants.ERROR_IMAGE)
    )
}

@Composable
private fun TitleBottom(
    communication : String = "Si tienes problemas comunicate con nosotros"
) {
    Text(
        text = communication,
        textAlign = TextAlign.Center,
        ...
        modifier = Modifier
            .wrapContentWidth()
            .testTag(Constants.ERROR_COMMUNICATION)
    )
}
